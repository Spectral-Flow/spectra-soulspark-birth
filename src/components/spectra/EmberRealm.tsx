...
const EmberRealm = () => {
  ...
  const handleLocationChange = (newLocation: string) => {
    ...
    const locationNarrations = {
      'Misty Chapel Ruins':
        'Ancient stones whisper of forgotten prayers. Ember materializes from the shadows.',
      'Whispering Forest Grove':
        'Trees lean in as if sharing secrets. The canopy filters ethereal light.',
      'Crystal Stream Crossing':
        'Water sings with crystalline voices. Reflections show more than just your image.',
      'Ancient Oak Circle':
        'Massive oaks form a natural cathedral. The air thrums with primal energy.',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card className="p-6 bg-card/50 backdrop-blur-sm border-primary/20 spectra-glow">
            ...
            <div className="mb-4">
              <p className="text-foreground leading-relaxed text-base">{currentNarration}</p>
            </div>
            ...
            <div className="space-y-3">
              {companions.map((companion) => (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mb-1">{companion.type}</p>
                  <p className="text-xs leading-relaxed">{companion.personality}</p>
                </div>
              ))}
            </div>
          </Card>
            ...
            <ScrollArea className="h-64">
              <div className="space-y-2">
                {adventureLog.map((log) => (
                  <div key={log.id} className="p-2 rounded border border-border/50 text-xs">
                    <div className="flex items-center gap-1 mb-1">
                      <Clock className="w-3 h-3" />
                      <span className="text-muted-foreground">
                        {log.timestamp.toLocaleTimeString()}
                      </span>
                      {log.memory && <Sparkles className="w-3 h-3 text-memory-vivid" />}
                    </div>
                    <p className="font-medium mb-1">{log.action}</p>
                    <p className="text-muted-foreground leading-relaxed">{log.result}</p>
                  </div>
                ))}
              </div>
            </ScrollArea>
